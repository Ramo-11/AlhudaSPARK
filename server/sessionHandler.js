...
async function loginUser(req, res) {
    ...
    if (!email || !password) {
        logger.error("Error logging in: email or password missing")
    ...
    try {
        ...
        if (!user) {
            logger.error("Error logging in: user not found")
            return res.status(401).send({ message: "User not found" })
        }

        const isMatch = await bcrypt.compare(password, user.password)
        if (!isMatch) {
            logger.error("Error logging in: incorrect password")
...
async function signupUser(req, res) {
    ...
    if (password !== confirmedPassword) {
        logger.error("Sign-up failed: Passwords do not match")
    ...
    try {
        ...
        if (existingUser) {
            logger.error("Sign-up failed: Email already in use")
